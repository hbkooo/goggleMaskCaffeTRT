    ...
    : network(network), nms_threshold(nms)
{
    ...
    trtNet->buildTrtContext(model + "/mnet.prototxt", model + "/mnet.caffemodel",
            model + "/mnet.engine");
...
#endif
    /* Load the network. */
    Net_.reset(new Net<float>((model + "/mnet.prototxt"), TEST));
    Net_->CopyTrainedLayersFrom((model + "/mnet.caffemodel"));
...
void RetinaFace::detect(Mat &img, float threshold, float &scale, vector<FaceDetectInfo> &faceInfo)
{
    ...
    std::cout << "loading caffe face test1 ...\n";
    std::cout << "resize.shape( " << resize.cols << ", " << resize.rows << " )" << std::endl;

    resize.convertTo(resize, CV_32FC3);
    cvtColor(resize, resize, CV_BGR2RGB);

    std::cout << "loading caffe face test2 ...\n";
    ...
    std::cout << "loading caffe face test3 ...\n";

    split(resize, input_channels);
    std::cout << "loading caffe face test34 ...\n";

    Net_->Forward();

    std::cout << "loading caffe face test4 ...\n";
